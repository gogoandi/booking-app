...
describe('AuthService', () => {
  ...
  it.each(['signUp', 'signInWithPassword'] as const)(
    'posts credentials to %s and saves the authenticated session',
    async (authType) => {
      const service = createService();
      const result = firstValueFrom(service.authRequest(authType, response.email, 'Password1!'));
      const request = httpTesting.expectOne({
        method: 'POST',
        url: `https://identitytoolkit.googleapis.com/v1/accounts:${authType}?key=test-api-key`,
      });
      ...
      expect(JSON.parse(localStorage.getItem('userData')!)).toEqual(savedUser);
  ...
  it.each([
    ...
    ['UNRECOGNIZED_ERROR', 'An unknown error has occurred'],
  ])('maps %s to a readable error without creating a session', async (code, message) => {
    const service = createService();
    const result = firstValueFrom(
      service.authRequest('signInWithPassword', response.email, 'wrong'),
    );

    httpTesting
      .expectOne((request) => request.method === 'POST')
      .flush({ error: { message: code } }, { status: 400, statusText: 'Bad Request' });
  ...
  it('handles network errors without a Firebase error payload', async () => {
    const service = createService();
    const result = firstValueFrom(
      service.authRequest('signInWithPassword', response.email, 'Password1!'),
    );

    httpTesting.expectOne((request) => request.method === 'POST').error(new ProgressEvent('error'));

    await expect(result).rejects.toThrow('An unknown error has occurred');
    expect(service.user.value).toBeNull();
    expect(localStorage.getItem('userData')).toBeNull();
  });
  ...
  it('restores a saved session on startup and logs out after its remaining lifetime', () => {
    ...
    vi.setSystemTime(new Date(+now + 1800000));
  ...
  it.each([
    ['expired', JSON.stringify({ ...savedUser, expirationDate: now.toISOString() })],
    ['invalid expiration', JSON.stringify({ ...savedUser, expirationDate: 'invalid' })],
    ['missing token', JSON.stringify({ ...savedUser, token: '' })],
    ['malformed JSON', '{invalid json'],
  ])('clears a saved session with %s data', (_description, storedValue) => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
    localStorage.setItem('userData', storedValue);
